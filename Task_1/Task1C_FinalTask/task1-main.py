...
for k in range(1,N_images+1):
	grid_map = [ [ -1 for i in range(grid_line_y-1) ] for j in range(grid_line_x-1) ]
	
	imgpath='task1sets/task1_img_'+str(k)+'.jpg'
	img_r = cv2.imread(imgpath)
	...
	for i in range(0,6):
                x,y=(550,50+100*i)
                if grid_map[i][5]>9 or grid_map[i][5]<0:
                        cv2.putText(img_r, str(grid_map[i][5]), (x-m/2, y+n/4),cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 4)
                else:
                        cv2.putText(img_r, str(grid_map[i][5]), (x-m/4, y+n/4),cv2.FONT_HERSHEY_PLAIN, 5, (0, 0, 255), 4)
	cv2.imshow('task1_img_'+str(k),img_r)
	cv2.imwrite('output/task1_img_'+str(k)+'.jpg',img_r)
	cv2.waitKey()
